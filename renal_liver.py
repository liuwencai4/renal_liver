...
#应用标题
st.title('A machine learning-based predictive model for predicting liver metastasis in patients with renal cancer')
...
lung_metastases = st.sidebar.selectbox("Lung metastases",('No','Yes'),index=0)
...
features = ['Grade','T','N','Tumor.Size','lung.metastases','Bone.metastases']
target = 'Liver.metastasis'
#处理数据不平衡
...
sp = 0.5
#figure
is_t = (XGB.predict_proba(np.array([[Grade,T,N,Tumor_Size,lung_metastases,Bone_metastases]]))[0][1])> sp
prob = (XGB.predict_proba(np.array([[Grade,T,N,Tumor_Size,lung_metastases,Bone_metastases]]))[0][1])*1000//1/10
